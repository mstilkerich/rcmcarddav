...
*/
  ...
  public function get_vcard_from_carddav($uid)
  {{{
    $rcmail = rcmail::get_instance();
    $user = $rcmail->user;;
    if (!file_exists(dirname(__FILE__) . "/config.".$user->data['username'].".inc.php"))
  ...
  {{{
    $vcf = $this->get_vcard_from_carddav($uid);
    if (!$vcf)
	    return false;
    $record = explode("+delim+", $uid);
    $id = $record[0];
  ...
  {{{
    ...
    $vcf = $this->get_vcard_from_carddav($oid);
    if (!$vcf)
	    return false;
    $vcfnew = preg_replace("/\nFN:.*?\r\n/", "\nFN:".$save_cols['name']."\r\n", $vcf);
    $vcfnew = preg_replace("/\nN:[^;]*;[^;]*;/", "\nN:".$save_cols['surname'].";".$save_cols['firstname'].";", $vcfnew);
    if (strlen($mail) < 1){
	    write_log('carddav', "No Mail address. adding ".$save_cols['email']);
	$vcfnew = preg_replace("/\nEND:VCARD/", "\nEMAIL;TYPE=HOME:".$save_cols['email']."\r\nEND:VCARD", $vcfnew);
	write_log('carddav', "VCF is now: $vcfnew");
    } else {
...
Updating 1217a3ab-3d18-493b-8da6-514a8d99a870 with array(4) {
  ["name"]=>
  string(43) "HONDA Mobil-Center Automobile & Motorräder"
  ["firstname"]=>
  string(43) "HONDA Mobil-Center Automobile & Motorräder"
  ["surname"]=>
  string(0) ""
  ["email"]=>
  string(19) "honda@anderdonau.de"
}
*/
  }}}
  ...
  {{{
	...
	$id = $this->guid();
	while ($this->get_vcard_from_carddav($id."+delim+")){
	    $id = $this->guid();
	}
    }

    if ($update){
	$vcf = $this->get_vcard_from_carddav($id);
  ...
  {{{
	...
	write_log("carddav", "Deleting. Old:\n$vcf\nNew:\n$vcfnew");
	if (!preg_match("/\nEMAIL/", $vcfnew)){
	    write_log("carddav", "No Mailaddress left. Deleting altogether.");

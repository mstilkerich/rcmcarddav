...
class carddav extends rcube_plugin
{
	...
	{{{
		...
		$retval = array(
			'options' => array(
				array('title'=> rcube_utils::rep_specialchars_output($this->gettext('cd_name','html', 'strict', true)), 'content' => $content_name),
				array('title'=> rcube_utils::rep_specialchars_output($this->gettext('cd_active','html', 'strict', true)), 'content' => $content_active),
				array('title'=> rcube_utils::rep_specialchars_output($this->gettext('cd_use_categories','html', 'strict', true)), 'content' => $content_use_categories),
				array('title'=> rcube_utils::rep_specialchars_output($this->gettext('cd_username','html', 'strict', true)), 'content' => $content_username),
				array('title'=> rcube_utils::rep_specialchars_output($this->gettext('cd_password','html', 'strict', true)), 'content' => $content_password),
				array('title'=> rcube_utils::rep_specialchars_output($this->gettext('cd_url','html', 'strict', true)), 'content' => $content_url),
				array('title'=> rcube_utils::rep_specialchars_output($this->gettext('cd_refresh_time','html', 'strict', true)), 'content' => $content_refresh_time),
			),
			'name' => $blockheader
		);
		...
		if (!$abook['presetname'] && preg_match('/^\d+$/',$abookid)) {
			...
			$retval['options'][] = array('title'=> rcube_utils::rep_specialchars_output($this->gettext('cd_delete','html', 'strict', true)), 'content' => $content_delete);
		}
	...
	{{{
		...
		if (version_compare(PHP_VERSION, '5.3.0') < 0) {
			$args['blocks']['cd_preferences'] = array(
				'options' => array(
					array('title'=> rcube_utils::rep_specialchars_output($this->gettext('cd_php_too_old','html', 'strict', true)), 'content' => PHP_VERSION)
				),
				'name' => rcube_utils::rep_specialchars_output($this->gettext('cd_title','html', 'strict', true))
			);
			return $args;
		}
		...
		if(!array_key_exists('_GLOBAL', $prefs) || !$prefs['_GLOBAL']['fixed']) {
			$args['blocks']['cd_preferences_section_new'] = $this->cd_preferences_buildblock(
				rcube_utils::rep_specialchars_output($this->gettext('cd_newabboxtitle','html', 'strict', true)),
				array(
					'id'           => 'new',
					'active'       => 1,
					'use_categories' => 1,
	...
	{{{
		...
		if (!isset($prefs['_GLOBAL']['hide_preferences']) || (isset($prefs['_GLOBAL']['hide_preferences']) && $prefs['_GLOBAL']['hide_preferences'] === FALSE)) {
			...
			$args['list']['cd_preferences'] = array(
				'id'      => 'cd_preferences',
				'section' => rcube_utils::rep_specialchars_output($this->gettext('cd_title','html', 'strict', true))
			);
		}
		return($args);
	}}}
	...
	{{{
		...
		foreach($abooks as $abook) {
			...
			if( isset($_POST[$abookid."_cd_delete"]) ) {
				...
				$newset = array (
					'name' => rcube_utils::get_input_value($abookid."_cd_name", RCUBE_INPUT_POST, false, NULL),
					'username' => rcube_utils::get_input_value($abookid."_cd_username", RCUBE_INPUT_POST, true, NULL),
					'url' => rcube_utils::get_input_value($abookid."_cd_url", RCUBE_INPUT_POST, false, NULL),
					'active' => isset($_POST[$abookid.'_cd_active']) ? 1 : 0,
					'use_categories' => isset($_POST[$abookid.'_cd_use_categories']) ? 1 : 0,
					'refresh_time' => rcube_utils::get_input_value($abookid."_cd_refresh_time", RCUBE_INPUT_POST, false, NULL),
				);

				// only set the password if the user entered a new one
				$password = rcube_utils::get_input_value($abookid."_cd_password", RCUBE_INPUT_POST, true, NULL);
		...
		$new = rcube_utils::get_input_value('new_cd_name', RCUBE_INPUT_POST, false, NULL);
		if ( (!array_key_exists('_GLOBAL', $prefs) || !$prefs['_GLOBAL']['fixed']) && strlen($new) > 0) {
			$srv    = rcube_utils::get_input_value('new_cd_url', RCUBE_INPUT_POST, false, NULL);
			$usr    = rcube_utils::get_input_value('new_cd_username', RCUBE_INPUT_POST, true, NULL);
			$pass   = rcube_utils::get_input_value('new_cd_password', RCUBE_INPUT_POST, true, NULL);
			$pass = self::$helper->encrypt_password($pass);
			$abname = rcube_utils::get_input_value('new_cd_name', RCUBE_INPUT_POST, false, NULL);
			$use_categories = intval(rcube_utils::get_input_value('new_cd_use_categories', RCUBE_INPUT_POST, true, NULL), 0);
			...
			if(is_array($srvs) && count($srvs)>0) {
				foreach($srvs as $srv){
					...
					self::insert_abook(array(
						...
						'url'      => $srv['href'],
						'refresh_time' => rcube_utils::get_input_value('new_cd_refresh_time', RCUBE_INPUT_POST, false, NULL)
					));
				}
			} else {
				$args['abort'] = true;
